...
from cVAE_architecture2 import loss_vae
...
def train_cVAE(model, train_loader, test_loader, beta, criteration = loss_vae, learning_rate=1e-3, optimizer = None, epochs=25, batch_size=128,  plot_history=True, latent_dim=16,
    ...
    def __plot_history(history):
        ...
        plt.ylabel('Loss')
        plt.legend()
        model_name = "cVAE_Name_zdim_" + str(latent_dim)+"_epochs_"+str(epochs)
    ...
    if save_model:
        model_name = "cVAE_Name_zdim_" + str(latent_dim)+"_beta_"+str(beta)+"_epochs_"+str(epochs)
