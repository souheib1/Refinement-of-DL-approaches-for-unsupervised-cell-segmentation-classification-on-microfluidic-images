...
def tSNE(z, latent_space, clustering_labels, true_labels=None, cmap='viridis', aff_true_labels=True,data=None):
    ...
    fig = plt.figure()
    fig, ax = plt.subplots(layout="constrained")
    scatter = ax.scatter(x=proj_2d[:, 0], y=proj_2d[:, 1], s=5, c=clustering_labels, alpha=0.5, cmap=cmap)
    ...
    if aff_true_labels:
        ...
        scatter = ax.scatter(x=proj_2d[:, 0], y=proj_2d[:, 1], s=5, c=true_labels, alpha=0.5, cmap=cmap)
        legend1 = ax.legend(*scatter.legend_elements(), loc="upper left", title="Classes", fontsize=5)
